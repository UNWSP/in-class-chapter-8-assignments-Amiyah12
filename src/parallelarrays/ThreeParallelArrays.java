...
public class ThreeParallelArrays {
	...
	public static void main(String[] args) {
		...
		while(true) {
			Strings state = JOptionPane.showInputDialog(null, "Pick a State name");
			int index=-1;
			for(int i=0; i<50; i++) 
			
			{if(states[i].equals(state)) {index=i; break;}}
			
			if(index! = -1)
			{
				String abb = StatesAbb
						String Capital = Capitals[index];
				JOptionPane.showMessageDialog(null, "What is the Abbreviation for  " +state+"and the coresponding capital is "+Capital+"\n");
